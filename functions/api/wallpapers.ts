...
export async function onRequest({ request }: { request: Request }) {
  ...
  if (!cid) return errorResponse("缺少参数: cid", 400);
  ...
  try {
    const params = new URLSearchParams({ c: "WallPaper", a: "getAppsByCategory", cid, start: String(start), count: String(count), from: "360chrome" });
    ...
    return jsonResponse({ errno: 0, errmsg: "成功", meta: { consume: json.consume, total: json.total, cid, start, count }, data });
  } catch (err: any) {
    return errorResponse("获取壁纸失败", 500, err.message);
  }
}

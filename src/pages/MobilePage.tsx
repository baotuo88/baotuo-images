...
type MobileCat = { id: string; name: string; is_hot?: boolean };
...
export default function MobilePage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const res =
          cid === null
            ? await mobileApi.latest(start, PAGE_SIZE, "new")
            : await mobileApi.byCid(cid, start, PAGE_SIZE, "new");
        const data = res.data || [];
        if (!canceled) {
          setItems(data);
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-6">
      <div className="mb-4 text-white/60">手机 / 竖屏专用 / 原图下载</div>
